...
class FileName {
    ...
    /** 生成 {PVA} 标记的值 */
    getPVA(postId) {
        const { P, V, A } = _Store__WEBPACK_IMPORTED_MODULE_1__.store.getPva(postId);
        const array = [];
        P && array.push(`${P}P`);
        V && array.push(`${V}V`);
        A && array.push(`${A}A`);
        ...
        if (_Lang__WEBPACK_IMPORTED_MODULE_2__.lang.type === 'zh-cn' || _Lang__WEBPACK_IMPORTED_MODULE_2__.lang.type === 'zh-tw' || _Lang__WEBPACK_IMPORTED_MODULE_2__.lang.type === 'ja') {
            // 中文、日文里，返回值如 【15P1V1A】
    ...
    getFileName(data) {
        ...
        for (const [key, val] of Object.entries(cfg)) {
            if (result.includes(key)) {
                ...
                let once = val.value === '' || val.value === null || val.value === undefined
                    ? ''
                    : String(val.value);
...
class Store {
    ...
    addResult(data) {
        ...
        for (const fileData of files) {
            ...
            if (_Config__WEBPACK_IMPORTED_MODULE_0__.Config.fileType.image.includes(ext)) {
                pva.P++;
            }
            else if (_Config__WEBPACK_IMPORTED_MODULE_0__.Config.fileType.video.includes(ext)) {
                pva.V++;
            }
            else if (_Config__WEBPACK_IMPORTED_MODULE_0__.Config.fileType.music.includes(ext)) {
                pva.A++;
            }
        }
...
class Download {
    ...
    async browserDownload(url, fileName, id, taskBatch) {
        ...
        const sendData = {
            ...
            taskBatch,
            // 同名文件冲突时的处理方式由用户设置决定
            conflictAction: _setting_Settings__WEBPACK_IMPORTED_MODULE_11__.settings.conflictAction,
        };
...
class DownloadControl {
    ...
    async saveErrorRecord(data, err) {
        ...
        const sendData = {
            ...
            fileName: recordName,
            // 同名文件冲突时的处理方式由用户设置决定
            conflictAction: _setting_Settings__WEBPACK_IMPORTED_MODULE_8__.settings.conflictAction,
        };
...
class SaveFanCard {
    ...
    loadMaskURL() {
        if (!this.config.assets.cardMask) {
            this.config.assets.cardMask = webextension_polyfill__WEBPACK_IMPORTED_MODULE_7___default().runtime.getURL('images/cardMask.png');
        }
    }
    config = {
        canvasWidth: 1280,
    ...
    async download(blob, fileName) {
        ...
        const sendData = {
            ...
            fileName,
            // 同名文件冲突时的处理方式由用户设置决定
            conflictAction: _setting_Settings__WEBPACK_IMPORTED_MODULE_6__.settings.conflictAction,
        };
        // 在 Firefox / Chrome 的隐私窗口里下载 blob 文件时，需要携带文件数据，
        // 详见 Config.sendBlob / sendDataURL 的注释
        ...
        webextension_polyfill__WEBPACK_IMPORTED_MODULE_7___default().runtime.sendMessage(sendData).catch(() => { });
    }
}
...
<strong>😊 Улучшен перевод некоторых текстов</strong>`,
    ...
    _文件名重复时的处理方式: [
        `文件名<span class="key">重复</span>时的处理方式`,
        `檔名<span class="key">重複</span>時的處理方式`,
        `How to handle <span class="key">duplicate</span> file names`,
        `ファイル名が<span class="key">重複</span>した場合の処理方法`,
        `파일명 <span class="key">중복</span> 시 처리 방법`,
        `Что делать, если имя файла <span class="key">повторяется</span>`,
    ],
    _文件名重复时的处理方式的说明: [
        `当你重复下载相同的文件时，硬盘上可能已经存在同名文件。你可以决定如何处理：<br>
- 添加序号：在本次下载的文件名后面添加一个序号，例如“(1)”，以避免文件名重复。<br>
- 覆盖已有文件：覆盖硬盘上的同名文件。<br>
- 手动处理：浏览器会显示另存为窗口，此时你可以自行决定如何处理：可以覆盖、可以修改文件名，也可以取消保存这个文件。<br>
<br>
谨慎使用“手动处理”：<br>
在另存为窗口显示期间，浏览器会被冻结，下载也会暂停。<br>
另外，如果下载的文件里有很多同名文件，那么浏览器会频繁弹出另存为窗口。<br>
所以请谨慎使用“手动处理”方式。`,
        `當你重複下載相同的檔案時，硬碟上可能已經存在同名檔案。你可以決定如何處理：<br>
- 新增序號：在本次下載的檔名後面新增一個序號，例如「(1)」，以避免檔名重複。<br>
- 覆寫已有檔案：覆寫硬碟上的同名檔案。<br>
- 手動處理：瀏覽器會顯示「另存新檔」視窗，此時你可以自行決定如何處理：可以覆寫、可以修改檔名，也可以取消儲存這個檔案。<br>
<br>
請謹慎使用「手動處理」：<br>
在「另存新檔」視窗顯示期間，瀏覽器會被凍結，下載也會暫停。<br>
另外，如果下載的檔案裡有很多同名檔案，瀏覽器會頻繁彈出「另存新檔」視窗。<br>
所以請謹慎使用「手動處理」方式。`,
        `When you download the same file again, a file with the same name may already exist on your disk. You can decide how to handle it:<br>
- Add a serial number: A serial number, such as "(1)", is added after the file name of this download to avoid duplicate file names.<br>
- Overwrite existing file: Overwrite the file with the same name on your disk.<br>
- Handle manually: The browser will show a "Save As" dialog. You can then decide how to handle it yourself: you can overwrite the file, change its file name, or cancel saving it.<br>
<br>
Use "Handle manually" with caution:<br>
While the "Save As" dialog is shown, the browser is frozen and downloads are paused.<br>
Also, if many of the files being downloaded have the same name, the "Save As" dialog will pop up frequently.<br>
So please use the "Handle manually" option with caution.`,
        `同じファイルを繰り返しダウンロードすると、ディスク上に同じ名前のファイルが既に存在する場合があります。処理方法を選択できます：<br>
- 連番を付ける：ダウンロードしたファイル名の末尾に「(1)」などの連番を追加し、ファイル名の重複を防ぎます。<br>
- 既存のファイルを上書き：ディスク上にある同じ名前のファイルを上書きします。<br>
- 手動で処理：ブラウザーに「名前を付けて保存」ダイアログが表示されるので、上書きするか、ファイル名を変更するか、保存をキャンセルするかを自分で決められます。<br>
<br>
「手動で処理」は慎重に使用してください：<br>
「名前を付けて保存」ダイアログが表示されている間、ブラウザーはフリーズし、ダウンロードも一時停止します。<br>
また、ダウンロードするファイルの中に同じ名前のものが多いと、「名前を付けて保存」ダイアログが頻繁に表示されます。<br>
そのため、「手動で処理」は慎重に使用してください。`,
        `같은 파일을 다시 다운로드하면 디스크에 이미 같은 이름의 파일이 있을 수 있습니다. 처리 방법을 선택할 수 있습니다:<br>
- 일련번호 추가: 다운로드한 파일 이름 뒤에 "(1)"과 같은 일련번호를 추가하여 파일 이름 중복을 방지합니다.<br>
- 기존 파일 덮어쓰기: 디스크에 있는 같은 이름의 파일을 덮어씁니다.<br>
- 수동으로 처리: 브라우저에 "다른 이름으로 저장" 창이 표시되며, 덮어쓰기, 파일 이름 변경, 저장 취소 등 처리 방법을 직접 결정할 수 있습니다.<br>
<br>
"수동으로 처리"는 주의해서 사용하세요:<br>
"다른 이름으로 저장" 창이 표시되는 동안 브라우저가 멈추고 다운로드도 일시 중지됩니다.<br>
또한 다운로드할 파일 중 같은 이름의 파일이 많으면 "다른 이름으로 저장" 창이 자주 표시됩니다.<br>
그러므로 "수동으로 처리" 방식은 주의해서 사용하세요.`,
        `Если вы повторно загружаете один и тот же файл, на диске может уже существовать файл с таким же именем. Вы можете выбрать способ обработки:<br>
- Добавить номер: к имени загружаемого файла добавляется номер, например «(1)», чтобы избежать дублирования имен файлов.<br>
- Перезаписать существующий файл: перезапишется файл с таким же именем на диске.<br>
- Обработать вручную: браузер покажет диалог «Сохранить как», и вы сможете сами решить, как поступить: перезаписать файл, изменить его имя или отменить сохранение.<br>
<br>
Используйте «Обработать вручную» с осторожностью:<br>
Пока открыт диалог «Сохранить как», браузер будет заблокирован, а загрузка приостановлена.<br>
Кроме того, если среди загружаемых файлов много файлов с одинаковыми именами, браузер будет часто показывать диалог «Сохранить как».<br>
Поэтому используйте вариант «Обработать вручную» с осторожностью.`,
    ],
    _添加序号: [
        `添加序号`,
        `新增序號`,
