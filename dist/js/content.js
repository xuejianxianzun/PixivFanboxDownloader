    ...
    listenEvents() {
        ...
        chrome.runtime.onMessage.addListener((msg) => {
            ...
            if (msg.msg === 'downloaded') {
                ...
                // 浏览器把文件保存到本地时出错
                _Log__WEBPACK_IMPORTED_MODULE_3__["log"].error(`${msg.data.url} download error! code: ${msg.err}.`);
...
const formHtml = `<form class="settingForm">
      ...
      <label for="fileType1" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.image.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_图片')}&nbsp;</label>
      
      <input type="checkbox" name="music" id="fileType2" class="need_beautify checkbox_common" checked>
      <span class="beautify_checkbox"></span>
      <label for="fileType2" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.music.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_音乐')}&nbsp;</label>

      <input type="checkbox" name="video" id="fileType3" class="need_beautify checkbox_common" checked>
      <span class="beautify_checkbox"></span>
      <label for="fileType3" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.video.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_视频')}&nbsp;</label>
      
      <input type="checkbox" name="compressed" id="fileType4" class="need_beautify checkbox_common" checked>
      <span class="beautify_checkbox"></span>
      <label for="fileType4" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.compressed.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_压缩文件')}&nbsp;</label>
      
      <input type="checkbox" name="ps" id="fileType5" class="need_beautify checkbox_common" checked>
      <span class="beautify_checkbox"></span>
      <label for="fileType5" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.ps.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_PS文件')}&nbsp;</label>

      <input type="checkbox" name="other" id="fileType6" class="need_beautify checkbox_common" checked>
      <span class="beautify_checkbox"></span>
      <label for="fileType6" title="${_Store__WEBPACK_IMPORTED_MODULE_1__["store"].fileType.other.join(',')}"> ${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_其他')}&nbsp;</label>
      </p>
...
const langText = {
    ...
    _命名标记title: ['投稿标题', '記事のタイトル', 'Article title', '投稿標題'],
    _命名标记tags: [
        '投稿的 tag 列表（可能为空）',
        '記事のタグリスト（空の場合があります）',
    ...
    _命名标记fee: ['投稿的价格', '記事価格', 'Article price', '投稿的價格'],
    _命名标记index: [
        '文件在它所属的投稿里的序号',
        '記事内のファイルのシリアル番号',
