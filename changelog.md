...
**已知问题：**
这个设置在手机浏览器里应该是无效的，因为手机浏览器似乎不会覆盖同名文件。根据我的经验：
- 一些浏览器会舍弃新的同名文件，不下载它
- 一些浏览器总是添加序号
...
之前如果用户排除了投稿里的一些文件（不下载它们），那么它们在 HTML 里的路径会使用 fanbox 上的原 URL。这经常会导致内容无法加载。
...
PS：该扩展无法在移动端 Firefox 上使用，因为该浏览器的下载功能是残废的。
