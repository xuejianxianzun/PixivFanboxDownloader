...
**已知问题：**
这个设置在手机浏览器里可能无效，或者行为不符合预期。这是因为手机浏览器似乎不会覆盖同名文件。

以移动版 Edge 浏览器为例：
- 选择添加序号：当有同名文件时，Edge 会跳过这个新文件，所以不会保存新文件，也不会产生序号
- 选择覆盖：Edge 此时反而为下载新文件，并为其添加序号。行为与字面描述不符
- 选择手动处理：情况同“添加序号”，不会保存新文件。Edge 也不会像电脑上的浏览器那样显示另存为窗口。
其他浏览器可能会使用其他处理方式，我没有测试。
...
早期一些文本是使用谷歌翻译机翻的，不准确，现在我让 AI 把所有语句都重新翻译了一遍。
